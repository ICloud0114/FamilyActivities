...
- (void)createContentView
{
    ...
    [choiceTableView addLegendHeaderWithRefreshingBlock:^{
        
        [weakSelf getChoiceData];
    }];
    
    
    [choiceTableView.header beginRefreshing];
...
//    [self.navigationItem setItemWithTitle:@"深圳" titleColor:[UIColor blackColor] fontSize:15.0f itemType:left];
    ...
    [self.navigationItem setItemWithTitle:@"精选活动" titleColor:[UIColor blackColor] fontSize:18.0f itemType:center];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    [cell.bgImage sd_setImageWithURL:[NSURL URLWithString:[dataArray objectAtIndex:indexPath.row][@"image"]] placeholderImage:[UIImage imageNamed:@"info_photo_img"]];
    ...
    if ([[dataArray objectAtIndex:indexPath.row][@"status"] isEqualToString:@""])
    {
        cell.dayNumber.hidden = YES;
        cell.timeIcon.hidden = YES;
    }
    else
    {
        cell.dayNumber.text =   [dataArray objectAtIndex:indexPath.row][@"status"];
    }
//
    cell.agreeNumber.text = [dataArray objectAtIndex:indexPath.row][@"count"];
    
    if ([[dataArray objectAtIndex:indexPath.row][@"distance"] isEqualToString:@""])
    {
...
//        [hud hide];
        ...
        if ([recivedDictionary[DATA][ACTDATA] isKindOfClass:[NSArray class]])
        {
            ...
            [choiceTableView.header endRefreshing];
        }
        else
        {
            [hud setCaption:@"没有数据"];
