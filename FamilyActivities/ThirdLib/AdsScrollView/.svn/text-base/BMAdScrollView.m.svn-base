...
//        backgroundView.center = CGPointMake(self.width/2.0f, self.height/2.0f);
        ...
        pageControl = [[ZZPageControl alloc]initWithFrame:CGRectMake(0, self.bounds.size.height - 35, 320, 20)];
        pageControl.backgroundColor = [UIColor clearColor];
        pageControl.hidden = YES;
        [pageControl setContentHorizontalAlignment:UIControlContentHorizontalAlignmentCenter];
...
- (void)setDataSource:(NSArray *)dataSource
{
    ...
    CGSize newSize = CGSizeMake(WIDTH * (_dataSource.count + 2),  imageSV.bounds.size.height);//设置scrollview的大小
    [imageSV setContentSize:newSize];
...
//            title = titleArr[i - 1]; 
            imageURL = [dataSource[i - 1] objectForKey:@"ImageUrl"];
        }
        if (i == 0) {
//            title = titleArr[dataSource.count - 1];
            imageURL = [dataSource[dataSource.count - 1]objectForKey:@"ImageUrl"];
        }else if(i == dataSource.count +1)
        {
//            title = titleArr[0];
            imageURL = [dataSource[0]objectForKey:@"ImageUrl"];
        }
        //创建内容对象
        CGRect titleRect = CGRectMake(0, 150, 320, 30);
        BMImageView *imageView =  [[BMImageView alloc]initWithImageName:imageURL title:title x:WIDTH*i tFrame:titleRect iHeight:imageSV.frame.size.height titleHidden:YES];
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView
{
    ...
    if (currentPage == 0)
    {
        [imageSV scrollRectToVisible:CGRectMake(WIDTH * [self.dataSource count],0,WIDTH,HEIGHT) animated:NO]; // 序号0 最后1页
    }
