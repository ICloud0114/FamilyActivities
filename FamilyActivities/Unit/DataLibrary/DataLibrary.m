...
//    actid imageUrl title site distance date
    ...
    {
        sql = [NSString stringWithFormat:@"create table if not exists %@ (id primary key,title varchar,imgUrl varchar,site varchar,actid varchar Unique,distance varchar,date varchar,agree varchar,type varchar,rUrl varchar)",tableName];
    }
...
- (id)getFirstDataByTable:(NSString *)tableName
{
    ...
    if ([tableName isEqualToString:FAVORITIES_TB])
    {
        while ([rs next])
        {
            ...
            model.rUrl = [rs stringForColumn:RURL];
...
//        sql = [NSString stringWithFormat:@"create table if not exists %@ (id primary key,title varchar,search_id,imgUrl varchar)",tableName];
    ...
    if ([tableName isEqualToString:FAVORITIES_TB])
    {
        sql = [NSString stringWithFormat:@"replace into %@ (title,imgUrl,site,actid,distance,date,agree,type,rUrl) values ('%@','%@','%@','%@','%@','%@','%@','%@','%@')",tableName, data[@"title"], data[@"image"], data[@"site"], data[@"actid"], data[@"distance"],data[@"date"],data[@"agree"],data[@"type"],data[RURL]];
    }
    else if([tableName isEqualToString:AGE_TB])
    {
...
- (BOOL)searchActivityIdByTable:(NSString *)tableName andActivityId:(NSString *)Id
{
    ...
    NSString *sql = [NSString stringWithFormat:@"select * from %@ where actid = '%@'",tableName,Id];
    ...
    while ([rs next])
    {
        FirstLevelAreaModel *model = [[FirstLevelAreaModel alloc]init];
        model.title = [rs stringForColumn:@"title"];
        model.level1_id = [rs stringForColumn:@"actid"];
        [arr addObject:model];
